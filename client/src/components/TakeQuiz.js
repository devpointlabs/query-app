...
import {Card, Grid, Segment, List, Button, Container } from 'semantic-ui-react';
...
const TakeQuiz = (props) => {
  ...
  const renderQuestions = (id) => {
    ...
    return (
      <>
        <Grid className="center-grid-answer">
          <Grid.Row></Grid.Row>
              <Grid.Row>Question {number + 1} of {questions.length}</Grid.Row>
                {<Question
                  c={question}  
                  next={next}
                  question_id = {id}
      ...
      <>
        { props.auth.user.role == 'teacher' ?
          <Link textAlign="center" style={{backgroundColor: "#7e6bc4", color: "white",}} to={`/quizzes/${props.match.params.id}/question_form`}>
      ...
      >Back
        ...
        <Button style={{backgroundColor: "#7e6bc4", color: "white",}}>Grade Answers</Button>
        </Link>
      { }
